    ...
    public function contarUsuarios($search = '')
{
    ...
    $builder->select('COUNT(DISTINCT ccp_usuario.id) as total') // Usamos DISTINCT para evitar duplicados
        ->join('ccp_persona', 'ccp_usuario.persona_id = ccp_persona.id', 'inner')
        ->join('ccp_persona_identificacion', 'ccp_persona_identificacion.id_persona = ccp_persona.id', 'left')
        ->join('ccp_email', 'ccp_email.persona_id = ccp_persona.id', 'left')
        ->where('ccp_usuario.estado', 1);
    ...
    public function registrarColaborador($colaboradorData, $roles, $emails, $telefonos)
{
    ...
    $personaData = [
        ...
        'distrito' => $colaboradorData['distrito'],
        'estado' => 1
    ];
    $db->table('ccp_persona')->insert($personaData);
    ...
    $db->table('ccp_persona_identificacion')->insert($identificacionData);
    ...
    public function obtenerColaboradorPorId($id)
{
    ...
    $builder->select('
        ...
        ccp_persona.apellido_materno,
        ccp_persona.direccion,
        ccp_persona.departamento,
        ccp_persona.provincia,
        ccp_persona.distrito,
    ...
    public function actualizarColaborador($id, $colaboradorData, $roles, $emails, $telefonos)
{
    ...
    // Actualizar datos en ccp_persona (incluye departamento, provincia y distrito)
    ...
    // Mantener usuario y estado actuales si no se envían nuevos valores
    $usuarioData = [
        'usuario' => $colaboradorData['usuario'] ?? $usuario['usuario'],
        'estado' => isset($colaboradorData['estado']) ? $colaboradorData['estado'] : $usuario['estado']
    ];

    // Validar y actualizar la contraseña solo si se ingresa una nueva
    if (!empty($colaboradorData['password']) && strlen($colaboradorData['password']) >= 8) {
        $usuarioData['password'] = password_hash($colaboradorData['password'], PASSWORD_BCRYPT);
    }
